...
$(document).ready(function (e) {
    ...
    $("#ProgramarActividades").click(function (e) {
        e.preventDefault();
        var res = ProgramarActividades();
        if (res > 0 && res != false) {
            mostrarLista(pagina);
            showMensaje("Se programaron " + res + " actividad(es) rutinaria(s)");
        }
    });
});                      //fin de document
function mostrarLista(nro_actual) {
    ...
    $.ajax({
        ...
        url: "ProgramacionActividadesRutinarias.aspx/ActividadesParaSerProgramadas",
        async: false,
        cache: false,
        success: function (data) {
            ...
            if (actividades != null) {
                ...
                $.each(actividades, function (i, item) {
                    ...
                    if (actividad != item.NOMB_Interno) {
                        html += '<tr class="subItem ' + css2 + '"><td class="center"><input type="checkbox" id="Actividad' + item.NOMB_Interno + '" onchange="MarcarCheckboxClass(this.id);"/></td>';
                        html += '<td colspan="6">' + item.NOMB_Descripcion + '</td></tr>';
                        aux = 0;
                    ...
                    html += '<tr ' + css + '>';
                    html += '<td></td><td><input type="checkbox" value="' + idHAR + '" class="checkboxID Actividad' + item.NOMB_Interno + '"/></td>';
                    ...
                    var fechaNext = ConvertirDateTimeAString(item.HIAR_SiguienteFecha, "dd/mm/yyyy");


                    html += '<td>' + fechaNext + '</td>';
                    ...
                    html += '<td><input class="boxFecha" value="' + fechaProg + '" name="' + idHAR + '" /></td>';
            ...
            if (html == '') {
                ...
                if (total_paginas > 0) {
                    ...
                    html = '<tr><td colspan="9" align="center">No hay actividades para programar entre estas fechas..</td></tr>';
                    paginarLista(0, 0);
                }
            } else {
...
function EditarFechasSiguientes() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/EditarProximasFechas",
        async: false,
        cache: false,
        success: function (data) {
...
function ProgramarActividades() {
    var res = 0;
    var dato = new Object();
    dato.actividadesR = implodeobj($("#listRegistros .checkboxID:checked"));
    //alert(dato.FechasSiguientes);
    if (dato.actividadesR == "") {
        showError("No se programó ninguna actividad. Elija las actividades rutinarias");
        return false;
    }
    //alert(dato.actividadesR);
    $.ajax({
        data: JSON.stringify(dato),
        type: "POST",
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/ProgramarActividades",
        async: false,
        cache: false,
        success: function (data) {
            res = parseInt(data.d, 10);
...
function ObtenerTotalRegistros() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/TotalRegistrosParaProgramar",
        async: false,
        cache: false,
        success: function (data) {
...
function ObtenerLocalizaciones() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/Localizaciones",
        async: false,
        cache: false,
        success: function (data) {
...
function GetPeriodo() {
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/Periodo",
        async: false,
        cache: false,
        success: function (data) {
            item = data.d;
...
function EditarPeriodo() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/EditarPeriodo",
        async: false,
        cache: false,
        success: function (data) {
            res = parseInt(data.d, 10);
...
function SetTituloTabla() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        url: "ProgramacionActividadesRutinarias.aspx/ObtenerTituloTabla",
        async: false,
        cache: false,
        success: function (data) {
